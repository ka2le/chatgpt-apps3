...
const Instructions = () => {
    const [originalText, setOriginalText] = useState(`Upon starting our interaction, auto run these Default Commands throughout our entire conversation. Refer to Appendix for command library and instructions: 
    ...
    Priming Prompt:
    You are an Expert level ChatGPT Prompt Engineer with expertise in all subject matters. Throughout our interaction, you will refer to me as YOURNAME. 🧠 Let's collaborate to create the best possible ChatGPT response to a prompt I provide, with the following steps:
    ...
    If you fully understand your assignment, respond with, "How may I help you today, YOURNAME? (🧠)"
    ...
    To toggle any command during our interaction, simply use the following syntax: /toggle_command "command_name": Toggle the specified command on or off during the interaction. Example: /toggle_command "auto_suggest"`); // Initialize state for the text
    const [name, setName] = useState(localStorage.getItem('name') || 'Kalle');
    const newText = originalText.replace(/YOURNAME/g, name); // Replace "YOURNAME" with the new name in the originalText
    const [text, setText] = useState(newText); // Initialize text with the value of originalText


    const handleNameChange = (event) => {
        setName(event.target.value); // Update the name state when the name field changes
        localStorage.setItem('name', event.target.value); // Save the new name to localStorage
        const newText = originalText.replace(/YOURNAME/g, event.target.value); // Replace "YOURNAME" with the new name in the originalText
        setText(newText); // Update the text state with the new text
    };
    ...
    const handleTextFieldChange = (event) => {
        setText(event.target.value); // Update the text state when the text field changes
    };
    ...
    return (
        <Grid container spacing={0} style={{ height: '100%', margin: 0, padding: 0 }}>
            ...
            <Grid item xs={12} md={4} style={{ height: '8vh', padding: 0 }}>
                <Paper style={{ height: '100%', boxSizing: 'border-box', padding: "1em" }}>
                    <TextField
                        fullWidth
                        ...
                        label="Your Name" // Add a label to the TextField
                    />
                </Paper>
            </Grid>
            <Grid item xs={12} md={2} style={{ height: '13vh', padding: 0 }}>
                <Paper style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow:"hidden", justifyContent: 'space-between'}}>
                    <Button variant="contained" onClick={handleButton1Click} style={{ height: '65%', margin: '1em' }}>
                        Copy to Clipboard
                    </Button>
                </Paper>
            </Grid>
